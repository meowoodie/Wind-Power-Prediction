...
def plot_wind_graph(t, locs, edges, latlim, lnglim, filename):
    ...
    plt.axis("off")
    fig.tight_layout()
    plt.savefig("../results/plot_graph_k20/%s-45.png" % filename)
...
if __name__ == "__main__":
    ...
    wind = np.load("../data/data_k40/sample_wind.npy") # [ T, K, 2 ]
    locs = np.load("../data/data_k40/locations.npy")   # [ K, 2 ]
    ...
    np.save("../data/data_k40/gsupp.npy", supp)
    ...
    for t in tqdm(range(n_time)):
        for j, i in zip(*np.nonzero(supp)):
            ...
            if angle <= 22.5 or 360 - angle <= 22.5:
                dgraph[t, j, i] = 1
    np.save("../data/data_k40/dgraph.npy", dgraph)
    ...
    # # generate gif
    # images    = []
    # filenmaes = [ "../results/plot_graph_k20/graph-plot-%d-45.png" % t for t in range(550) ]
    # for filename in filenmaes:
    #     images.append(imageio.imread(filename))
    # print("[%s] generating the graph animation" % arrow.now())
    # imageio.mimsave('../results/graph-animation-k20.gif', images)